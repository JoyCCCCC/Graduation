  ...
  <div>
    ...
    >
        ...
        <div class="add-cont-part pt-18 plr-18">
          ...
          >
            <el-form-item label="产品">
              ...
              >
                <el-option
                  v-for="productItem in productTypeData"
                  :key="productItem.key"
                  :label="productItem.value"
      ...
      <el-form-item label="项目所在区域">
        <!-- 回填数据暂无map  暂时是输入框  后期修改  -->
        <!-- <el-cascader
          v-model="formData.area"
          class="cascader-container"
          placeholder="请选择/搜索项目所在区域"
          :options="areaConfig"
          :props="{ label: 'value',value:'key',checkStrictly: true }"
          filterable
        /> -->
        <el-input
          v-model="formData.area"
...
import { secretRelatedOptions } from '@/views/project/constant'
import { fetchCustomerData, fetchEmployeeData, fetchApolloData } from '@/api/dict'
...
export default {
  ...
  methods: {
    ...
    // 获取行业信息
    getIndustryData() {
      fetchApolloData({ key: 'industry' }).then(res => {
        if (res) {
    ...
    // 获取产品类型信息
    getProductTypeData() {
      fetchApolloData({ key: 'productType' }).then(res => {
        if (res) {
